...
#include "name_list.h"
 ...
 void insert_name_list(sNAME_LIST *name_list,
                      sNODE *node)
{
   //method is called in whie looop; reads names.text and called line by line until end of file
   //append nodes (that were read from the text file) ,that contains names
   // to a name list, which has a list of nodes that has names 
   if (name_list != NULL){ //list has never been created ; NOT EMPTY BUT NEVER CREATED!
      ...
      for (int i = 0; i < listCount; i++) //iterate the list
      {
         ...
         else if (i == listCount - 1) // end of the list and append
         {
            currentNode->next = node;
            name_list->rear = node;
            name_list->count++;
         }
         currentNode = currentNode->next; //move to next name list
      }
   } else {
